Create one horizontal animation strip for Codex pet `jaehee`, state `jumping`.

User override for this run: strictly match `references/canonical-base.png` for identity, proportions, palette, outline weight, face, green star pouch, green strap, and small head tuft. For this state, remove the mint microphone entirely; do not carry it in any frame. Make the jump more expressive: body squashes then lifts, floppy ears bounce upward/downward, and the small chestnut-like head tuft pops upward slightly while remaining attached/visually belonging to the head. No floor, shadow, landing marks, dust, or detached effects.
