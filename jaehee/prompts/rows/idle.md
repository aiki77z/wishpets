Create one horizontal animation strip for Codex pet `jaehee`, state `idle`.

User override for this run: strictly match `references/canonical-base.png` for identity, proportions, palette, outline weight, face, green star pouch, green strap, and small head tuft. Idle is the only state allowed to keep and use the mint microphone: make the puppy look like it is softly singing, with a happy singing mouth/tiny tongue and a gentle rhythmic body bob. The microphone may move subtly with the paw. Do not add musical notes, lyrics, speech bubbles, text, or detached effects beyond the existing simple belly music-note motif from the base.
