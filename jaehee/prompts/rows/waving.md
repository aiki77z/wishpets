Create one horizontal animation strip for Codex pet `jaehee`, state `waving`.

User override for this run: strictly match `references/canonical-base.png` for identity, proportions, palette, outline weight, face, green star pouch, green strap, and small head tuft. For this state, remove the mint microphone entirely; do not carry it in any frame. Use a slightly larger waving gesture through one paw only, with ears reacting subtly to the motion. Do not draw wave marks, sparkles, text, or detached effects.
