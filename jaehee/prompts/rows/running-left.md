Create one horizontal animation strip for Codex pet `jaehee`, state `running-left`.

User override for this run: strictly match `references/canonical-base.png` for identity, proportions, palette, outline weight, face, green star pouch, green strap, and small head tuft. For this state, remove the mint microphone entirely; do not carry it in any frame. Show leftward drag movement with a bigger readable animation: ears swing, body leans/travels toward screen-left, tiny feet alternate, and the tail wags across frames. No speed lines, dust, shadows, or detached effects.
