Create one horizontal animation strip for Codex pet `jaehee`, state `running`.

User override for this run: strictly match `references/canonical-base.png` for identity, proportions, palette, outline weight, face, green star pouch, green strap, and small head tuft. For this state, remove the mint microphone entirely; do not carry it in any frame. This is active task processing, not directional jogging: make the tail wag visibly, ears bounce, body bob/lean with focused effort, and paws move as if busily working near the body. Do not show literal sprinting across the frame, speed lines, dust, shadows, or detached effects.
