Create Codex pet row `running-right` for `kuri`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Use the uploaded references as identity: dark blue-gray chibi cat, large glossy black eyes with small white highlights, black simple whiskers, tiny black nose and cat mouth, pink spiral cheek marks, pink paw pads, chest paw pad, chestnut cap on head, pink heart patch in left ear with black double ring detail, light green star-like patch in right ear; no red crossbody accessory and no clothing.. Preserve silhouette, face, palette, material, proportions, markings, and props.
